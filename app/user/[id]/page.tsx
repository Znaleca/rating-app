    ...
    if (notFound) return (
        <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans">
            ...
            <div className="flex flex-col items-center justify-center h-[80vh] gap-6">
                <FaUserCircle size={48} className="text-[var(--foreground)]" />
                <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[var(--muted-foreground)]">User not found</p>
                <Link href="/search" className="text-[9px] font-black uppercase tracking-widest text-yellow-400 hover:text-[var(--foreground)] transition-colors flex items-center gap-2">
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans selection:bg-yellow-400 selection:text-[var(--background)]">
            ...
            <main className="w-full px-6 xl:px-16 pt-28 pb-12 max-w-7xl mx-auto">
                ...
                {/* Back link */}
                <Link href="/search" className="inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-[var(--muted-foreground)] hover:text-yellow-400 transition-colors mb-12">
                    <FaArrowLeft size={8} /> Back to Search
                </Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16 pb-16 border-b border-[var(--border-subtle)]">
                    ...
                    <div className="lg:col-span-7 flex items-start gap-8">
                        ...
                        <div>
                            <p className="text-[9px] font-black uppercase tracking-[0.5em] text-[var(--muted-foreground)] mb-2">Public Archive</p>
                            ...
                            return (
                                ...
                                >
                                    <div className="relative w-14 h-20 shrink-0 overflow-hidden bg-[var(--foreground)]/5 border border-[var(--border-subtle)]">
                                        {item.poster_url ? (
                                            <Image src={item.poster_url} alt={item.title || ""} fill sizes="56px" className="object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                                        ) : (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-4">
                        {filteredItems.map(item => {
                            ...
                            return (
                                ...
                                >
                                    {item.poster_url ? (
                                        <Image src={item.poster_url} alt={item.title || "Unknown"} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, (max-width: 1536px) 25vw, 14vw" className="object-cover transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
                                    ) : (
                                    ...
                                    {item.rating !== null && (
                                        <div className="absolute top-2 right-2 flex items-center gap-1 bg-[var(--background)]/80 backdrop-blur-sm px-2 py-1 border border-yellow-400/30">
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-3">
                                        <p className="text-[9px] font-black text-[var(--foreground)] uppercase leading-tight line-clamp-2 mb-1">{item.title}</p>
                                        <div className="flex items-center gap-1.5">
                                            <MediaIcon size={7} className="text-[var(--muted-foreground)]" />
                                            <span className="text-[7px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest">{item.media_type}</span>
                                        </div>
                                    </div>
