            ...
            <div className="mb-10 border-b border-white/5 pb-8 flex items-end justify-between gap-6">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-500">Activity</span>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-white">Reviews</h1>
                    <p className="text-slate-500 text-sm mt-2 font-medium">All ratings and reviews submitted across the platform</p>
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                    <p className="text-4xl font-black text-white">{reviews?.length ?? 0}</p>
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">Total Entries</p>
                </div>
            </div>
            ...
            {error ? (
                ...
                <div className="space-y-3">
                    {reviews.map((rev) => {
                        ...
                        return (
                            <div key={rev.id} className="border border-white/5 bg-white/[0.015] p-5 hover:bg-white/[0.03] transition-colors group">
                                <div className="flex items-start gap-5">
                                    {/* Avatar */}
                                    <div className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center text-[10px] font-black text-slate-400 shrink-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 mb-2">
                                            ...
                                            <span className="text-slate-700 text-[9px]">•</span>
                                            <span className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">{rev.media_type}</span>
                                        ...
                                        {rev.review && rev.review.trim() !== "" ? (
                                            ...
                                            <p className="text-slate-700 text-xs italic">No review text — rating only</p>
                                        )}

                                        {/* Footer */}
                                        <div className="flex items-center gap-4 mt-3">
                                            {rev.status && (
                                                <span className="text-[8px] font-black uppercase tracking-widest text-slate-600 border border-white/5 px-2 py-1">
                                                    {rev.status}
                                                </span>
                                            )}
                                            <span className="text-[8px] text-slate-700 font-bold ml-auto">
